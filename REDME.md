...
1. **git pull**
   ...
   **git clone**
   - 役割：リモートのデータを丸ごとローカルに複製
   - コマンド：`git clone [リポジトリのURL]`

   GitHubからURLを取得: GitHubの自分のリポジトリページを開き、緑色の 「<> Code」 ボタンをクリックして、SSHのURL（git@github.com:...）をコピーします。

   git branch -a
# 赤字で remotes/origin/... と出ればデータは存在しています

git checkout [ブランチ名]
# これだけでリモートの枝を元に、ローカルブランチが作成されます

💡 Tips: なぜ最初から見えないのか 実務ではブランチが数百本に及ぶこともあるため、Gitは「データは全部持ってくるが、作業場所（展開）は必要なものだけにする」という効率的な管理を行っています。
...
Gitが「自動でマージできない！」と判断する基準は、主に以下の2つです。

### 1. 「同じファイルの同じ行」を同時に書き換えたとき
Gitはファイルを行単位で管理しています。
- ブランチA：10行目を「Red」に書き換え
- ブランチB：10行目を「Blue」に書き換え
この場合、Gitは「Red」と「Blue」のどちらが正しいか判断できないため、人間に判断を仰ぎます。

### 2. 「一方が削除し、もう一方が編集」したとき
- ブランチA：`config.js` を削除した
- ブランチB：`config.js` の中身を書き換えた
この場合も、「消すべきか、残して更新すべきか」が不明なためコンフリクトになります。

### 💡 コンフリクトが「起きない」ケース
逆に、以下の場合はGitが自動で合体させてくれます。
- 違うファイルを編集しているとき
- 同じファイルでも、編集している「行」が離れているとき

### コンフリクトを防ぐための心得
- プルリクは小さく出す: 一度に大量のファイルを変更すると、それだけ衝突のリスクが高まる。
- コミュニケーションをとる: 同じファイルを触る予定がある時は、事前に声をかけ合う。
- こまめな更新: 自分のブランチに、こまめに main の最新状態を取り込む（git pull origin main）。
